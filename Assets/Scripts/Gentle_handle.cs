    ...
    IEnumerator Boid_control()
    {
        while(true)
        {
            ...
            if (transform.GetChild(0).gameObject.GetComponent<Boid>().localSwarm.Count > 0 | vboid != Vector3.zero)
            {
                ...
                /* Ici on a un probleme de repere car il va aligner la consigne avec le vectoeur Forward on non le vecteur up de notre boid */
                transform.LookAt(vboid, transform.up);
